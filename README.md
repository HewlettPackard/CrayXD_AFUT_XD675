...
Playbooks : Ansible playbooks are defined with the tasks required to update the different components of system firmware, fetch system firmware inventory for the supported Cray XD servers and a playbook to view power states or to power on or to power off Cray XD675 nodes.
...
This repository contains ansible modules, playbooks to perform firmware upgrade HPE Cray XD675
...
3. power_state_XD675.yml: Playbook to fetch the power state information, to power on, to power off the Cray XD675 nodes.
...
For HPE Cray XD675 supported targets are:
...
2. Update the config.ini
   ...
   - update_image_path_XD675 : Path to local hpm file for HPE Cray XD675
...
# Get Power State of the Cray XD675 nodes

The playbook `power_state_XD675.yml` is used to fetch the power state, power on, power off the Cray XD675 nodes.
